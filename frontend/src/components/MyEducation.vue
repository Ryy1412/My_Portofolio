...
onMounted(async () => {try { const response = await
...
text-blue-600">{{ edu.period }}</p><h3 class="text-2xl font-bold textgray-800">{{ edu.institution }}</h3><p class="text-gray-600">{{
edu.major }}</p></div>
<div class="w-1/2 flex justify-start"><div class="w-4 h-4
bg-blue-600 rounded-full z-10"></div></div>
</div>
<div v-else class="w-full flex">
<div class="w-1/2 flex justify-end"><div class="w-4 h-4 bgblue-600 rounded-full z-10"></div></div>
